...
//		,obj_ext : '.ogz'
	...
	$.fn.popupRelationHPO_text2hpo = function (method) {
		if (methods[method]) {
				...
				$.each(values.sort(function(a,b){
					...
					if(isNumeric(this.count)){
						if(options.formatNumber){
							$number_html = $('<span>').addClass(current_settings.cssLinkNumberClass).text(this.count).css({'display':'inline-block','vertical-align':'middle'});
						}
						else{
							text += ' ('+this.count+')';
						}
					...
					if(isString(options.classname) && options.classname === CSS_PREFIX+current_settings.keySubclass){
						...
						if($number_html) $number_html.appendTo($('<'+current_settings.nodeName+'>').css({'display':'table-cell','vertical-align':'middle','text-align':'right','width':'1px'}).appendTo($link_base));
					}
		...
		function addOKCancelButtons(){
			...
			return $button_base;
                ...
                function addCloseButton(){
                        var $button_base = $('<'+current_settings.nodeName+'>').addClass(current_settings.cssButtonBaseClass);
                        //$button_base.css({'margin-right':'10px'}); 
                        var language = current_settings.language[getCurrentLanguage()];
                        var $button = $('<button>').addClass('btn').addClass('btn-default').data(OBJECT_KEY,  $.extend(true, {},{'exec' : 'close'})   ).text(language['close'] ? language['close'] : 'close').appendTo($button_base);
                        //$button.css({'height':'24px','padding-right':'4px','padding-left':'4px','font-family':"\'Times New Roman\', serif"});
			$button.css({'height':'26px','padding-right':'8px','padding-left':'8px','font-size':'14px'});
                        $button.on('click',executionClose);
                        return $button_base;
                }

		function addLangButton() {
			var $button_base = $('<'+current_settings.nodeName+'>').addClass(current_settings.cssButtonBaseClass).addClass('mydropdown');
                        var language = current_settings.language[getCurrentLanguage()];
			if(runSearchOptions.hasJA){			
				var $button = $('<button id=\"lang_btn\" class=\"btn btn-default droppbtn\">').text(language['jpn'] +' ▼').appendTo($button_base);
                        	//$button.css({'height':'24px','padding-right':'4px','padding-left':'4px','font-family':"\'Times New Roman\', serif"});
				$button.css({'height':'26px','padding-right':'8px','padding-left':'8px','font-size':'14px'});
			}else{
                        	var $button = $('<button id=\"lang_btn\" class=\"btn btn-default droppbtn\">').text(language['eng'] +' ▼').appendTo($button_base);
                        	//$button.css({'height':'26px','padding-right':'8px','padding-left':'8px','font-family':"\'Times New Roman\', serif"});
				$button.css({'height':'26px','padding-right':'8px','padding-left':'8px','font-size':'14px'});
			}

			let $div = $('<div class=\"mydropdown-content\">').appendTo($button_base);
			let $a_eng = $('<a>'+language['eng']+'</a>').appendTo($div);
                        $a_eng.on('click',_change_lang_eng);
			let $a_jpn = $('<a>'+language['jpn']+'</a>').appendTo($div);
			$a_jpn.on('click',_change_lang_jpn);


                        return $button_base;
		}
		
		function _change_lang_eng(){
			var language = current_settings.language[getCurrentLanguage()];
			$('#lang_btn').html(language['eng'] + '&nbsp;▼');
			runSearchOptions.hasJA = false;
			if(executionLanguage_timeoutID){
                        	clearTimeout(executionLanguage_timeoutID);
                        }
                        executionLanguage_timeoutID = setTimeout(function(){
                ...
                function _change_lang_jpn(){
                        var language = current_settings.language[getCurrentLanguage()];
			$('#lang_btn').html(language['jpn'] + '&nbsp;▼');
			runSearchOptions.hasJA = true;
                        if(executionLanguage_timeoutID){
                                clearTimeout(executionLanguage_timeoutID);
                        }
                        executionLanguage_timeoutID = setTimeout(function(){
...
//						$.PopupRelationHPOTokenTooltip();
					...
					onReady: function(){
						...
						$('#popup-hierarchy-hpo').hide();
						$ul.hide();
                                               
                                                //$('div.popup-hierarchy-hpo-top-bar > div > div > div:nth-child(1)').hide();
                                                //$('.popup-hierarchy-hpo-class-content-base').find('.popup-hierarchy-hpo-self-content').hide();
			...
			if($.isArray(results[current_settings.keySelfclass]) && results[current_settings.keySelfclass].length){
				...
				$.each(results[current_settings.keySelfclass], function(){
					...
					if($title && title_text_arr.length === 0){
						...
						var $title_td2 = $('<'+current_settings.nodeName+'>').css({'display':'table-cell','text-align':'right','width':'40px','white-space':'nowrap'}).appendTo($title_tr);
                                                
						/*var $selectedphenotype_title_td_right = $('<'+current_settings.nodeName+'>').css({'display':'inline-block','text-align':'right','padding-right':'8px','position':'relative'}).appendTo($title_td2);
                                               ...
                                               $language_select.prev('button').html($language_select.find('option[name='+getCurrentLanguage()+']').prop('selected', true).text()+'&nbsp;▼');
						*/
						addLangButton().appendTo($title_td2);
						addCloseButton().appendTo($title_td2);
					}
...
*/
		...
		$.PopupRelationHPOWithWebGL_text2hpo = function(query, options, settings){
			...
			if(!isObject(window.category2obj)){
				...
				category2obj_jqxhr = $.ajax({
					...
					setTimeout(function(){ $.PopupRelationHPOWithWebGL_text2hpo(query, options, settings); }, 0);
